...
public class Field {
  ...
  boolean hasOptionalArgs() {
    return getArgs().stream().filter(arg -> arg.isOptional()).count() > 0;
  }

  /** Returns the list of optional argument of this field */
  List<InputObject> getOptionalArgs() {
    if (optionalArgs == null) {
      optionalArgs = args.stream().filter(arg -> arg.isOptional()).toList();
    }
    return optionalArgs;
  }

  List<InputObject> getRequiredArgs() {
    return args.stream().filter(arg -> !arg.isOptional()).toList();
  }
